...
modelo_log <- train(
  ...
  family = binomial,
  trControl = ctrl,
  tuneGrid  = data.frame(df = c(2,3))
)

plot(modelo_log$finalModel)
# Avaliação no conjunto de teste
predicoes_prob <- predict(modelo_log, newdata = teste,type = "prob",s=modelo_log$bestTune)
predicoes_class <- predict(modelo_log, newdata = teste,type = "raw",s=modelo_log$bestTune)
